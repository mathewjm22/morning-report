  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-white shadow-2xl z-50 flex flex-col">
        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between bg-gradient-to-r from-purple-50 to-indigo-50">
        ...
        <div className="px-4 py-2 border-b border-slate-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-sm border border-slate-300 rounded focus:outline-none focus:border-indigo-500"
              autoFocus
            />
          </div>
        </div>
              ...
              <div className="divide-y divide-slate-100">
                {(f.body || []).map((row, i) => (
                  <div key={i} className="px-3 py-2 flex gap-3">
                    {row.letter && (
                      <div className="w-6 h-6 bg-indigo-100 text-indigo-800 rounded flex items-center justify-center font-bold text-xs flex-shrink-0">
                        {row.letter}
                      </div>
                    )}
