  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-full max-w-md bg-white shadow-2xl z-50 flex flex-col">
        <div className="px-4 py-3 border-b border-stone-200 flex items-center justify-between bg-gradient-to-r from-purple-50 to-sage-50">
          <div className="flex items-center gap-2">
            <Brain size={18} className="text-sage-700" />
            <div>
              <h2 className="text-sm font-semibold text-stone-900">Frameworks</h2>
              <p className="text-xs text-stone-500">Reference cards for building differentials</p>
            </div>
          </div>
          <button onClick={onClose} className="text-stone-500 hover:text-stone-800 p-1 rounded hover:bg-white">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="px-4 py-2 border-b border-stone-100">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-sm border border-stone-300 rounded focus:outline-none focus:border-sage-500"
              autoFocus
            />
          </div>
        </div>
        ...
        {/* Category tabs */}
        <div className="px-3 py-2 border-b border-stone-100 flex gap-1 overflow-x-auto">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          {filtered.length === 0 && (
            <p className="text-sm text-stone-400 italic text-center py-8">No matches.</p>
          )}
          {filtered.map(f => (
